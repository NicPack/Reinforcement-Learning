...
class Corner:
    def __init__(self, name: str) -> None:
        self.name = name
        self.image: np.ndarray = mpl_image.imread(f"lab2/zakrety/corners/{name}.png")
...
class Car:
    POTENTIAL_ACTIONS: list[Action] = [
        Action(ax, ay) for ax, ay in itertools.product((-1, 0, 1), (-1, 0, 1))
    ]
...
class Environment:
    ...
    def _random_start(self) -> Position:
        return random.sample(sorted(self.corner.starting_positions), 1)[0]
...
class Experiment:
    ...
    penalties: Optional[list] = (
        None  # tutaj będą się gromadzić kary przyznane w kolejnych epizodach
    )

    def run(self) -> None:
        self.penalties = []
        for _ in tqdm(
            range(self.number_of_episodes),
            desc=f"step_no: {self.driver.step_no} | step_size: {self.driver.step_size}",
        ):
    ...
    def visualize_optimal_policy(self, num_episodes: int = 5):
        self.driver.set_visualization_mode(True)

        for viz_episode in range(num_episodes):
            positions = []
            car = self.environment.spawn_car(self.driver)
            while True:
                positions.append(car.position())
            ...
            utils.draw_episode(
                ...
                self.environment.corner.image,
                positions,
                f"optimal_viz_{viz_episode}",
            )

        self.driver.set_visualization_mode(False)
